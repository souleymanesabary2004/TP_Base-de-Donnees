...
-- 5) Factures entre 1 200€ et 2 000€ en attente 
...
-- 6) Véhicules de type 'fourgon' avec capacité ≤ 3 500 kg
SELECT id_veh, immatriculation, type_veh, capacite_veh
FROM VEHICULES
WHERE type_veh = 'fourgon' AND capacite_veh <= 3500
ORDER BY capacite_veh DESC;

-- 7) Conducteurs dont le nom commence par 'D'
...
-- 11) Nombre de commandes par client et (> 3)
...
-- 14) Nombre de missions par destination (> 2)
...
-- 15) Missions avec conducteur et véhicule affectés
...
-- 18) Missions avec équipements associés
...
-- 19) Conducteurs sans véhicule/mission affecté (LEFT JOIN → NULL)
...
-- 20) Liste des équipements avec leur mission et distance
...
-- 21) Factures avec nom du client et statut de paiement
...
-- 22) Clients ayant des factures en attente
...
-- 23) Missions dont la distance est > à TOUTES les missions annulées
SELECT m.*
FROM MISSIONS m
...
-- 24) Conducteurs sans aucune affectation 
...
-- 25) Véhicules affectés à au moins une mission planifiée
...
-- 26) Clients n’ayant pas passé de commande (NOT EXISTS)
SELECT c.id_cl, c.nom_cl
FROM CLIENTS c
WHERE NOT EXISTS (
...
-- 27) Clients ayant au moins une commande confirmée
...
-- 31) Conducteurs ayant suivi une formation (formés) avec leur mentor
...
-- 32) Conducteurs sans aucune formation (pas de mentor renseigné)
SELECT d.num_permis, d.nom_cond, d.prenom_cond, d.type_permis
FROM CONDUCTEURS d
